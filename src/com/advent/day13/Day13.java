...
public class Day13 {
    ...
    public static void main(String[] args) {
        try {
            ...
            int busNumber = 0;
            int minutesToDeparture = 0;
            int timestamp = Integer.parseInt(br.readLine());
            System.out.println(timestamp);
            String[] parts = br.readLine().split(",");
            int[] intervals = Arrays.stream(parts).filter(s -> !s.equals("x")).mapToInt(num -> Integer.parseInt(num)).toArray();

            for (int interval : intervals) {
                int departureTime = timestamp + interval - (timestamp % interval);
                System.out.println("interval = " + interval + ", departureTime = " + departureTime);
                if (minDepartureTime == 0 || departureTime < minDepartureTime ) {
                    minDepartureTime = departureTime;
                    minutesToDeparture = departureTime - timestamp;
                    busNumber = interval;
                }
            }
            System.out.println("----------------------------------------------------------------");
            System.out.println("minDepartureTime = " + minDepartureTime);
            System.out.println("minutesToDeparture = " + minutesToDeparture);
            System.out.println("busNumber = " + busNumber);

            System.out.println("-------------------SOLUTION-------------------------------------");
            System.out.println(minutesToDeparture * busNumber);
            System.out.println("----------------------------------------------------------------");
